...
В этом случае:
\begin{equation*}
	u_t = f'\brs{\dfrac{x}{\sqrt{t}}} \cdot \brs{\dfrac{-x}{2t^{3/2}}} 
	\qquad u_x = f'\brs{\dfrac{x}{\sqrt{t}}} \cdot \dfrac{1}{\sqrt{t}} 
	\qquad u_{xx} = f''\brs{\dfrac{x}{\sqrt{t}}} \cdot \dfrac{1}{t}
\end{equation*}

Подставляем в уравнение:
\begin{align*}
	&-\dfrac{x}{2t\sqrt{t}} f'(z) = a^2 f''(z) \cdot \dfrac{1}{t}\\
	&\frac{f''(z)}{f'(z)} = - \frac{1}{a^2} \frac{x}{2\sqrt t} = -\frac{1}{2 a^2} z\\
	%a^2f''(z) &= -\dfrac{z}{2}f'(z) \\
	&\ln|f'(z)| = -\dfrac{z^2}{4a^2} + \tilde{C}_1
\end{align*}
