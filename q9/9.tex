...
Найдём такое решение $u(t, x) = f\brs{\dfrac{x}{\sqrt{t}}} = f(z)$. \\
Тогда:
\begin{equation*}
	u_t = f'\brs{\dfrac{x}{\sqrt{t}}} \cdot \brs{\dfrac{-x}{2t^{\frac{3}{2}}}}, \; \; u_x = f^{'}\brs{\dfrac{x}{\sqrt{t}}} \cdot \dfrac{1}{\sqrt{t}},\; \; u_{xx} = f^{''}\brs{\dfrac{x}{\sqrt{t}}} \cdot \dfrac{1}{t}
\end{equation*}
Подставляем в первое уравнение задачи:
\begin{equation*}
	-\dfrac{x}{2t\sqrt{t}} f^{'}\brs{\dfrac{x}{\sqrt{t}}} = a^2 f^{''}\brs{\dfrac{x}{\sqrt{t}}} \cdot \dfrac{1}{t} \Leftrightarrow a^2f^{''}(z) = -\dfrac{z}{2}f^{'}(z) \Rightarrow \ln|f^{'}(z)| = -\dfrac{z^2}{4a^2} + \tilde{C}_1
\end{equation*}
...
Тогда:
\begin{equation*}
	u(t, x) = f\brs{\dfrac{x}{\sqrt{t}}} = \dfrac{1}{\sqrt{\pi}} \int\limits_{-\infty}^{\frac{x}{\sqrt{4 t a^2}}} e^{-\mu^2}\;d \mu = \dfrac{1}{2}\sbrs{1 + \Phi\brs{\dfrac{x}{\sqrt{4 a^2 t}}}}
\end{equation*}
\begin{enumerate}
