...
% &=\frac{1}{2}(v^2_t)_t - (a v_t v_x)_x-(a v_t v_y)_y + \brs{\frac{1}{2}a^2v^2_x}_t+\brs{\frac{1}{2}a^2v^2_y}_t = &&\\ 
% &=\brs{\frac{v^2_t+a^2v^2_x+a^2v^2_y}{2}}_t+(-a^2 v_t v_x)_x+(-a^2 v_t v_y)_y = &&\\
% &= F^{(t)}_t + F^{(x)}_x + F^{(y)}_y \quad \text{---} \quad\text{дивергентный вид.}
% \end{flalign*}
\begin{flalign*}
  \qquad 0\;\; &= v_t v_{tt} - a^2 v_t v_{xx}-a^2 v_t v_{yy}= &&\\
  &=\frac{1}{2}\brs{v^2_t}_t + a^2 v_x v_{xt}-\brs{a^2 v_t v_x}_x + a^2 v_y v_{yt} - \brs{a^2 v_t v_y}_y = &&\\ 
  &=\frac{1}{2}\brs{v^2_t}_t - \brs{a v_t v_x}_x - \brs{a v_t v_y}_y + \brs{\frac{1}{2}a^2v^2_x}_t+\brs{\frac{1}{2}a^2v^2_y}_t = &&\\ 
  &=\brs{\frac{v^2_t+a^2v^2_x+a^2v^2_y}{2}}_t+\brs{-a^2 v_t v_x}_x + \brs{-a^2 v_t v_y}_y = &&\\
...
\end{multline*}
%hspace нужно для ээ выравнивания промежуточных строчек
