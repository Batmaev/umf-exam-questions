...
\begin{itemize}
  ...
  \begin{lemma}
    Пусть $K$ -- полярное ядро такое, что $\norm{\lambda K} < 1$. Тогда $\forall a, b \in C(\overline{G}) \longrightarrow \angleBr{(I - \lambda K)^{-1}a; b} = \int_{G}(I - \lambda K)^{-1}a(x)b(x)dx$ -- регулярная функция при $\abs{\lambda} < \norm{K}^{-1}$. Если дополнительно выполнено $\abs{\lambda}\cdot\norm{K^{'}} < 1$, то $\squareBr{(I - \lambda K)^{-1}a; b} = \squareBr{a; (I - \lambda K^{'})^{-1}b}$.
  \end{lemma}
  \begin{proof}
    $$ (I - \lambda K)^{-1}a(x) = \sum_{j = 0}^{\infty}\lambda^{j}K^{j}a(x) \Longrightarrow \int_{G}(I - \lambda K)^{-1}a(x)b(x)dx = \int_{G} \underbrace{\sum_{j = 0}^{\infty}\lambda^{j}K^{j}(a(x))b(x)}_{\text{сход. равномерно}}dx = \sum_{j = 0}^{\infty}\lambda^{j}\squareBr{\int_{G}K^{j}(a(x))b(x)dx}$$
    ...
    Докажем теперь вторую часть:
     ...
     Тогда:
     $$\angleBr{(I - \lambda K)^{-1}a; b} \overset{*}{=} \sum_{j=0}^{\infty}\lambda^{j}\angleBr{K^{j}a; b} = \sum_{j=0}^{\infty}\lambda^{j}\angleBr{a; (K^{'})^{j}b} = \angleBr{a; \sum_{j=0}^{\infty}\lambda^{j}(K^{'})^{j}b} = \angleBr{a; (I - \lambda K^{'})^{-1}b}$$
      Докажем (*):
     $$ \angleBr{Ku; v} = \int_{G}\int{G}K(x,y)u(y)dyv(x)dx =$$
     $$=  \int_{G}u(y)\squareBr{\int{G}K(x,y)v(x)d(x)}dy = \squareBr{x \rightarrow y; y \rightarrow x; (**)} =$$
     $$= \int_{G}u(x)\squareBr{\int{G}\underbrace{K(y,x)}_{K^{'}(x,y)}v(y)dy}dx = \angleBr{u; K^{'}v}$$
  ...
  \begin{proof}
    ...
    $$\hat{\mu}^{'}_{ij} = \angleBr{a_{i}; (I - \lambda Q^{'})^{-1}b_{j}} = \angleBr{(I - \lambda Q)^{-1}a_{i}; b_{j}} = \hat{\mu}_{ji}$$
