...
Если ввести обозначение $v_1(x) = \displaystyle\int_0^x u_1(y)\;dy + C$, то при $x \geq 0$

\begin{wrapfigure}[7]{r}{0.35\textwidth}
    \centering
    \includegraphics[width=0.35\textwidth]{./pic 6_1.pdf}
\end{wrapfigure}
...
\begin{wrapfigure}[7]{r}{0.35\textwidth}
    \includegraphics[width=0.35\textwidth]{./pic 6_2.pdf}
\end{wrapfigure}
...
\Subsection{Условия согласования}
...
\begin{align*}
    ...
    \cancel{\frac{1}{2}u'_0(0)} + \frac{1}{2a}u_1(0) &&\Rightarrow& u_1(0) &= 0 \\
     g''(+0) &= g''(-0) &\Leftrightarrow&& \frac{1}{2} u''_0(0) - \cancel{\frac{1}{2a} u_1'(0)} &= -\frac{1}{2}u_0''(0) - \cancel{\frac{1}{2a} u_1'(0)} &&\Rightarrow& u_0''(0) &= 0
\end{align*}
...
\begin{theorem}
    Пусть в смешанной задаче (\ref{6_1}) функции $u_0(x)$ и $u_1(x)$ таковы, что 
    \begin{itemize}
    ...
    \end{itemize}
    Тогда задача (\ref{6_1}) имеет единственное классическое решение $u(t,x) \in C^2(t \geq 0, x\geq 0)\colon$
\end{theorem}
