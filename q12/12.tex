...
\begin{equation*}
	\begin{cases}
		...
		\eval{u}_{x=0} = 0, \quad \eval{u}_{x=l} = 0, \quad & 0 < t < T;
	\end{cases}
\end{equation*}
...
\end{proof}
\begin{lemma}
	Оператор $A = -\dfrac{d^2}{dx^2}$, определенный на $\mathrm{D}(A)$, является симметричным относительно скалярного произведения в $\mathbb{L}_2\brk*{[0,l]}: (u,v) = \displaystyle\int\limits_0^l u(x) \overline{v(x)} dx$
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}
	Пусть $v(x) \in C^1\brk*{[0,l]}, v(0) = v(l) = 0$. 
...
\begin{proof}
	\begin{enumerate}
		\item Система $\brk[c]*{e_k} = \brk[c]*{\sin\brk*{\dfrac{\pi k}{l}x}}$ ортогональна относительно скалярного произведения в $\mathbb{L}_2\brk*{[0,l]}$, так как состоит из собственных функций оператора <<$-\Delta$>> с однородными условиями Дирихле~--- симметричного в $\mathbb{L}_2\brk*{[0,l]}$ оператора.
		...
		\item Ряд $\displaystyle\sum\limits_{k=1}^{+\infty} \abs*{\alpha_k}^2 < +\infty$ по неравенству Бесселя, т.к. система $\brk[c]*{g_k} = \brk[c]*{\cos\brk*{\dfrac{\pi k}{l}x}}$ ортогональна относительно скалярного произведения в $\mathbb{L}_2\brk*{[0,l]}$, так как состоит из собственных функций оператора <<$-\Delta$>> с однородными условиями Неймана~--- симметричного в $\mathbb{L}_2\brk*{[0,l]}$ оператора.
